...
module.exports = async function({workspace, dependencies, taskUtil, options}) {
    ...
    const log = await import("@ui5/logger").then(logger => logger.getLogger("builder:customtask:updateManifestToRelativeUrls"));
    ...
    const updateManifestDatasourceUri = function(sapAppBlock, pathPrefix) {
    ...
    await Promise.all(manifestFiles.map( manifestFile =>
        manifestFile.getString().then( manifestFileContent => {
            ...
            var updatedManifestFile = resourceFactory.createResource({ path: manifestFile.getPath(), string: JSON.stringify(manifestJsonContent, null, 4)});
